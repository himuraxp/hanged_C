...
int main(int argc, char* argv[])
{
	char letter = 0; // Stores the proposed lettre user
       	char secret_word[] = "PENDU"; // Word find
	int letter_found[] = {0} ;
	int letter_search[5] = {0}; // Boolean array. Each box corresponds to a letter from the secret word. 0 = not found letter, 1 = letter found
	int shots_remaining = 10; // Counter to shots remaining
	int i = 0;

	printf("=========================\n\nBienvenue dans le Pendu !\n\n=========================\n");

	while (shots_remaining > 0 && !win(letter_search))
	{
		...
		for (i = 0 ; i < 5 ; i++)
		{
			if (letter_search[i]) // If we found the letter n°i
